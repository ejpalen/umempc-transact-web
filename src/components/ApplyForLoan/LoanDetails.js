...
const LoanDetails = ({loanAmount, setLoanAmount,selectedLoanTerm, setSelectedLoanTerm, selectedLoanType, setSelectedLoanType,selectedPaymentMethod, setSelectedPaymentMethod
    ,loanTerms,loanTypes,paymentMethod
}) => {
  const navigate = useNavigate();

  
  useEffect(() =>{
    console.log(selectedLoanTerm)
  }, [selectedLoanTerm])
  ...
  return (
    <div className="wrapper text-default full-screen">
      ...
      <img
        ...
        />
        <h2 className="flex-1 text-center text-2xl text-bold">Loan Details</h2>
        
      </section>
      <main className=" flex-1 flex flex-col mt-14">
        <section className="flex flex-col gap-1">
            <section className="px-4 flex flex-col">
        <label className="mb-1" htmlFor="loanAmount">Loan Amount</label>
        <input type="tel" id="loanAmount" name="loanAmount" placeholder="Type amount here"
        value={loanAmount}
        onChange={(e) => setLoanAmount(e.target.value)}
        />
          <label className="mb-1 mt-6" htmlFor="loanTerms">Loan Term</label>
          <select name="loanTerms" id="loanTerms" onChange={(e) => setSelectedLoanTerm(e.target.value)}>
  {loanTerms.map((term, index) => (
    <option
    
    key={index} value={term} >{term}</option>
  ))}
</select>
</section>
<section className="px-4 flex flex-col">
