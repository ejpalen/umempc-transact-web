...
const PersonalDetails = ({name, setName,address, setAddress,contactNumber, setContactNumber,selectedCollege, setSelectedCollege
    ,selectedMembershipStatus, setSelectedMembershipStatus,selectedEmploymentStatus, setSelectedEmploymentStatus,
    membershipStatus,employmentStatus,colleges
}) => {
  ...
  return (
    <div className="wrapper text-default full-screen">
      <div className="support-bottom-nav fixed bottom-0 left-0 right-0 py-3 px-0 pt-0 z-10">
        ...
        className={`text-white text-xl text-bold p-2 flex-1 text-center bg-primary w-full mb-2 rounded-lg`}>
              Next
            </span>
        </nav>
      </div>
      ...
      <img
        ...
        />
        <h2 className="flex-1 text-center text-2xl text-bold">Personal Details</h2>
      ...
      <main className=" flex-1 flex flex-col mt-14">
        ...
        />
         <label className="mb-1 mt-6" htmlFor="contactNumber">Contact Number</label>
        <input type="tel" id="contactNumber" name="contactNumber" placeholder="Type your contact number"
        value={contactNumber} onChange={(e) => setContactNumber(e.target.value)}
        />
          <label className="mb-1 mt-6" htmlFor="address">Address</label>
        <textarea
        className="min-h-14"
        type="text" id="address" name="address" placeholder="Type your address"
        value={address} onChange={(e) => setAddress(e.target.value)}
        />
         <label className="mb-1 mt-6" htmlFor="college">College</label>
         <select name="college" id="college" defaultValue={selectedCollege} onChange={(e) => setSelectedCollege(e.target.value)}>
          ...
          {membershipStatus.map((status, index) => (
            <span
              ...
              className={`text-nowrap text-base rounded-lg py-1.5 px-6  text-center ${
                selectedMembershipStatus === status ? "button-1" : "button-2"
              }`}
          ...
          {employmentStatus.map((status, index) => (
            <span
              ...
              className={`text-nowrap text-base rounded-lg py-1.5 px-6 flex-1 text-center ${
                selectedEmploymentStatus === status ? "button-1" : "button-2"
              }`}
